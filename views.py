...
def wishes(request, status):
    ...
    wishes = chain(wish_user, wish_other)
    ...
    if status is not False:
        ...
        st = False
    #тут нам надо для каждого пожелания вытягивать дату последнего изменения и делать всплывающую подсказку
    #пока сделали только дату заказа. Так добавим же просто еще одну колонку для каждого заказа - дата последнего изменения,
    #которая будет обновляться при каждом изменении статуса. 
    c = {'wishes':wishes, 'page_name':u'Список заказов', 'user':request.user, 'status':False, 'newww':True}
    c.update(csrf(request))
